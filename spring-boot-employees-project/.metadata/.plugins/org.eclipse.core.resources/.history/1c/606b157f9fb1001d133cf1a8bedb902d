...
<tr>
<td><h4>${employee.first_name} ${employee.last_name}</h4><br>#${employee.emp_id}</td>
<td><p>${employee.mobile_num}<br>${employee.email_id}</p></td>
<td>${employee.emp_level}<br>
${employee.post_name}
</td>
<td><a href="${contextRoot}/addemployee/${employee.emp_id}">Edit </a> / <a href="/${employee.emp_id}">Delete </a></td>
